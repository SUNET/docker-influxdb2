...
FLAGS="--bolt-path ${INFLUXDB}.influxdbv2/influxd.bolt --engine-path ${INFLUXDB}.influxdbv2/engine --http-bind-address 0.0.0.0:8086 --log-level debug --reporting-disabled --tls-cert=${CERTDIR}/${HOSTNAME}-chain.crt --tls-key=${CERTDIR}/${HOSTNAME}.key"
